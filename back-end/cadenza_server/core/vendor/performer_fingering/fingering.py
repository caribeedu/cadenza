...
# Use it by calling:
#
#    fingered_notes = compute_fingering(notes, 'right')
#
# or
#
#    fingered_notes = compute_fingering(notes, 'left')
#
...
from cadenza_server.core.vendor.performer_fingering import cost as _cost_mod

_cost_databases: tuple[dict[str, float], dict[str, float]] | None = None


def _get_cost_databases() -> tuple[dict[str, float], dict[str, float]]:
    global _cost_databases
    if _cost_databases is None:
        _cost_databases = _cost_mod.createCostDatabase()
    return _cost_databases
...
def compute_fingering(notes: list[Any], left_or_right: str) -> list[dict[str, list]]:
...
def computeRightHandCostLookup(n1, n2, f1, f2):
    key = '%d,%d,%d,%d' % (n1, n2, f1, f2)
    rh, _ = _get_cost_databases()
    return rh[key]
...
def computeLeftHandCostLookup(n1, n2, f1, f2):
    key = '%d,%d,%d,%d' % (n1, n2, abs(f1), abs(f2))
    _, lh = _get_cost_databases()
    return lh[key]
...
def calcCost(current_node, previous_node, left_or_right):
    if left_or_right == 'left':
        costFunction = computeLeftHandCostLookup
    else:
        costFunction = computeRightHandCostLookup
