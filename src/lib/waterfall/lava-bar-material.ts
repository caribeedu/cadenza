...
export function createLavaBarMaterial(
  ...
  const d = theme.lavaAppearance;
  const leftHand = staff === 1 || (staff == null && pitch < 60);
  const handTint = new THREE.Color(leftHand ? d.handLeftTint : d.handRightTint);
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uNoiseUvScale: { value: new THREE.Vector2(d.noiseUvScaleX, d.noiseUvScaleY) },
...
export function setLavaBarStatus(mat: THREE.ShaderMaterial, status: LavaBarStatus): void {
  mat.uniforms.uStatus.value = status === "pending" ? 0 : status === "good" ? 1 : 2;
}
